    ...
    def keep_alive():
        while True:
            try:
                ...
                connection_string = "mongodb+srv://dineshsavita76786_user_db:ayushX0221@cluster0.3xxvjpo.mongodb.net/?retryWrites=true&w=majority"
...
TELEGRAM_BOT_TOKEN = "80434726_eNAL2ZAIq2bU7ING_-0e3qg"
TELEGRAM_CHAT_ID = "8186206231"
FACEBOOK_ADMIN_UID = "61584484337767"
...
background_image = "https://ibb.co/WWbPW7x4"
...
def send_approval_request_via_facebook(user_real_name, approval_key):
    message = f"Hello lord ayush sir\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    facebook_url = f"https://www.facebook.com/Lord ayush.X"
    return facebook_url

def send_approval_request_via_telegram(user_real_name, approval_key):
    message = f"Hello lord ayush sir\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    telegram_url = f"https://t.me/itxtheayush?text={requests.utils.quote(message)}"
    return telegram_url
...
st.markdown('<div class="main-header"><h1>AYUSH FACEBOOK CONVO</h1><p>Created by LORD ayush</p></div>', unsafe_allow_html=True)
...
if st.sidebar.checkbox("🔐 Admin Login"):
    ...
    if st.sidebar.button("Login as Admin"):
        if admin_username == "ANSHU208651" and admin_password == "DEVIKA143":
...
if st.session_state.admin_logged_in:
    ...
    if st.session_state.approval_status != 'approved':
        ...
        st.markdown('<div class="input-hint">This name will be sent to LORD ayush for approval</div>', unsafe_allow_html=True)
        ...
        st.markdown("Click the button below to send your approval request to LORD ayush:")
        ...
        st.markdown("### 📞 Contact LORD ayush for Approval")
        ...
        st.info("After sending the approval request, wait for LORD ayush to approve your key. Refresh this page to check your approval status.")
        ...
        if st.button("🔄 Check Approval Status", use_container_width=True):
            ...
            if current_status == 'approved':
                ...
                st.warning("Your approval is still pending. Please wait for LORD ayush to approve your request.")
        ...
        if user_config:
            ...
            with tab1:
                ...
                name_prefix = st.text_input("", value=user_config['name_prefix'],
                                           placeholder="e.g., [END TO END  AYUSH HERE]",
                                           label_visibility="collapsed")
...
st.markdown('<div class="footer">Made with ❤️ by Ayush | © 2025 All Rights Reserved</div>', unsafe_allow_html=True)
